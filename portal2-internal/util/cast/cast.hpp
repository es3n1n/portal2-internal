...
namespace util {
    namespace detail {
        ...
        template <typename T> concept is_clamp_castable = is_binary_radix<T> && ieee754_compliant<T>;

        template <typename T>
        requires is_clamp_castable<T>
    ...
    template <typename T>
    requires detail::is_clamp_castable<T>
    ...
    template <typename to_t, typename from_t>
    constexpr to_t clamp_cast(const from_t from) {
